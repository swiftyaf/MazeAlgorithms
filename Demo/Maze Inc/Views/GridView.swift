...
struct GridView: View {
    ...
    @Binding var cellActivity: (generated: [Cell], evaluating: [Cell])?
            ...
            ForEach(0..<(grid.rows * grid.cols), id: \.self) { index in
                ...
                if let cell = grid[row, col] {
                    let activeMode: CellActiveMode = {
                        if cellActivity?.generated.contains(cell) ?? false {
                            return .active
                        } else if cellActivity?.evaluating.contains(cell) ?? false {
                            return .preparing
                        } else {
                            return .none
                        }
...
#Preview {
    GridView(
        ...
        cellActivity: .constant((generated: [], evaluating: []))
    )
}
